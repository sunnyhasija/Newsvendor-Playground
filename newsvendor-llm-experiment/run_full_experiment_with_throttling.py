...
class FullExperimentRunnerWithThrottling:
    ...
    def _analyze_by_model(self, results: List[NegotiationResult]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if buyer_model not in model_stats:
                model_stats[buyer_model] = {
                    "as_buyer": {"count": 0, "successes": 0, "avg_price": 0, "total_tokens": 0},
                    "as_supplier": {"count": 0, "successes": 0, "avg_price": 0, "total_tokens": 0}
                }
            
            model_stats[buyer_model]["as_buyer"]["count"] += 1
            model_stats[buyer_model]["as_buyer"]["total_tokens"] += result.total_tokens
            
            if result.completed and result.agreed_price:
                model_stats[buyer_model]["as_buyer"]["successes"] += 1
                model_stats[buyer_model]["as_buyer"]["avg_price"] += result.agreed_price
            
            # Analyze supplier model
            supplier_model = result.supplier_model
            if supplier_model not in model_stats:
                model_stats[supplier_model] = {
                    "as_buyer": {"count": 0, "successes": 0, "avg_price": 0, "total_tokens": 0},
                    "as_supplier": {"count": 0, "successes": 0, "avg_price": 0, "total_tokens": 0}
                }
            
            model_stats[supplier_model]["as_supplier"]["count"] += 1
            model_stats[supplier_model]["as_supplier"]["total_tokens"] += result.total_tokens
            
            if result.completed and result.agreed_price:
                model_stats[supplier_model]["as_supplier"]["successes"] += 1
                model_stats[supplier_model]["as_supplier"]["avg_price"] += result.agreed_price
        ...
        for model, stats in model_stats.items():
            for role in ["as_buyer", "as_supplier"]:
                if stats[role]["count"] > 0:
                    ...
                    stats[role]["success_rate"] = 0
                    stats[role]["avg_tokens"] = 0
        ...
        for pattern, stats in reflection_stats.items():
            if stats["count"] > 0:
                ...
                if stats["successes"] > 0:
                    stats["avg_price"] = stats["total_price"] / stats["successes"]
                else:
                    stats["avg_price"] = 0
    ...
    def _analyze_costs(self, results: List[NegotiationResult]) -> Dict[str, Any]:
        ...
        cost_analysis = {
            ...
            "by_model": {},
            "by_negotiation": 0,
            "remote_vs_local": {"remote_cost": 0, "local_cost": 0, "remote_negotiations": 0, "local_negotiations": 0}
        }
        ...
        for result in results:
            ...
            for model in [result.buyer_model, result.supplier_model]:
                if model not in cost_analysis["by_model"]:
                    cost_analysis["by_model"][model] = 0
                cost_analysis["by_model"][model] += negotiation_cost / 2  # Split cost between buyer and supplier
                
                # Track remote vs local
                if model in remote_models:
                    cost_analysis["remote_vs_local"]["remote_cost"] += negotiation_cost / 2
                    cost_analysis["remote_vs_local"]["remote_negotiations"] += 0.5
                else:
                    cost_analysis["remote_vs_local"]["local_cost"] += negotiation_cost / 2
                    cost_analysis["remote_vs_local"]["local_negotiations"] += 0.5
